  ...
  <div class="screen" :id="'screen-' + screenNumber">
    ...
    >
      <input
        name="file_screen"
        type="file"
        :id="'input__file_screen-' + this.screenNumber"
        class="input input__file"
        @change="setImg($event.target)"
      />
      <label :for="'input__file_screen-' + this.screenNumber" class="input__file-button">
...
export default {
  ...
  methods: {
    ...
    elementDelete(curElement) {
      ...
      let elems = document.querySelectorAll(".screen__elements");
      for (let i = 0; i < elems.length; i++) {
        if (!elems[i].innerHTML) elems[i].parentNode.remove();
      }
    },
    screenDel() {
