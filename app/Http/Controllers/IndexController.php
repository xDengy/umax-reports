...
class IndexController extends Controller
{
    ...
    public function getPdf($report)
    {
        ...
        <section class="report">
            ...
            <p class="report__item__text">'.$curReport['report']->link.'</p>
            </div>
        ...
        foreach($curReport['elements']->values as $key => $element) {
            $element = $element[0];
            if($key > 0) {
                $pageCount += count(array_column($element, 'count'));
            }
            $elementHtml .= '
            <li>
                <a href="">'.($key + 1).'. '.$element['title'].'</a>
                <div class="content__list__border"></div>
                    ...
                    foreach($elementValue['elements'] as $elementBlock) {
                        switch($elementBlock['type']) {
                            ...
                            case 'img':
                                ...
                                if($img) {
                                    $elements .= '<img src="'.$elementBlock['value'][0].'" alt="">';
                                }
                                if($span) {
